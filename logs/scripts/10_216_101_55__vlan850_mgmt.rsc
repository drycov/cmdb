...
# Device IP: 10.216.101.55
# VLAN ID: 850
# Interface: vlan850_mgmt
# Matched subnet: 10.216.101.0/24
# Expected gateway: 10.216.101.1
# Target prefix: /24
# ============================================================
...
:foreach i in=[/ip address find where interface="vlan850_mgmt"] do={
    ...
    :if ($slashPos != nil) do={
        ...
        :if ($currentPrefix != "24") do={
            /ip address set $i address=($iponly . "/24")
        }
    }
}
...
:foreach r in=[/ip route find where dst-address="0.0.0.0/0"] do={
    ...
    :if ($gw != "10.216.101.1") do={
        /ip route remove $r
    }
}

:if ([:len [/ip route find where dst-address="0.0.0.0/0" and gateway="10.216.101.1"]] = 0) do={
    /ip route add disabled=no dst-address=0.0.0.0/0 gateway=10.216.101.1
}
...
/routing id add disabled=no id=10.216.101.55 name=rd1_850 select-dynamic-id=only-vrf select-from-vrf=main
