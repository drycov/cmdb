...
if TYPE_CHECKING:
    ...
    from app.topology.models import TopologyAnalysisResult
...
from domain.status_builder import StatusBuilder
from logging_setup import setup_logging
from models import AuditResult, Credentials
from report.writers.base import ReportWriter
...
from services.config_backup import ConfigBackupResult, ConfigBackupService
...
from services.targeted_remediation import TargetedRemediationResult, TargetedRemediator
...
class DeviceAuditor:
    ...
    def audit_device(self, ip: str) -> AuditResult:
        ...
        primary_result = self._process_session(
            ip=ip,
            result=result,
        ...
        fallback_result = self._process_session(
            ip=ip,
            result=result,
...
class AuditApplication:
    ...
    async def run_audit_command(self, *, show_progress: bool = True) -> None:
        ...
        if self.config.max_targets > 0:
            ips = ips[: self.config.max_targets]
        if not ips:
    ...
    async def generate_scripts_for_targets(
        ...
        if self.config.max_targets > 0 and not ips:
    ...
    async def backup_configs_for_targets(
        ...
        *,
        ...
        written: list[str] = []
        progress = (
            tqdm(total=len(target_ips), desc="Backing up configs")
            if show_progress
    ...
    async def upload_firmware_for_targets(
        ...
        *,
        ...
        progress = (
            tqdm(total=len(target_ips), desc="Uploading firmware")
            if show_progress
            else contextlib.nullcontext()
        )
        with progress as pbar:
    ...
    async def export_custom_report(
        ...
        writers = _build_report_writers(self.config, self.logger)
        if not writers:
    ...
    async def run_topology_command(
        ...
        *,
        ...
        ips = [ip] if ip else self.get_target_ips()
        if self.config.max_targets > 0 and ip is None:
            ips = ips[: self.config.max_targets]
        ...
        progress = (
            tqdm(total=len(ips), desc="Collecting topology") if show_progress else contextlib.nullcontext()
        )
        with progress as pbar:
    ...
    def _collect_topology_result(
        ...
        session = self.auditor.ssh.open_session(ip, self.auditor.primary_credentials)
        status = "ssh_ok"
        if session is None:
            session = self.auditor.ssh.open_session(ip, self.auditor.fallback_credentials)
            status = "fallback_ok"
...
def _build_report_writers(config: AppConfig, logger: logging.Logger) -> list[ReportWriter]:
    ...
    if (
        ...
        try:
            ...
            writers.append(GSheetWriter(spreadsheet=spreadsheet, batch_size=500))
        except Exception as exc:
